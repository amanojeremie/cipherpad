...
use cipherpad::{Cipherpad, EncryptedPad, NodeTree, Pad, PadMap, PadNode};
...
async fn encrypt_file_to_pad(
    ...
    if let (Some(pool), Some(master_key)) = (&cipherpad.pool, &cipherpad.master_key) {
        match encrypted_pad
            .encrypt_file_to_pad(pool, master_key, &file)
            .await
        {
            Ok(_) => Ok(()),
...
async fn decrypt_pad_to_file(
    ...
    if let (Some(pool), Some(master_key)) = (&cipherpad.pool, &cipherpad.master_key) {
        match encrypted_pad
            .decrypt_pad_to_file(pool, master_key, &file)
            .await
        {
            Ok(_) => Ok(()),
    ...
    if let (Some(pool), Some(master_key)) = (&cipherpad.pool, &cipherpad.master_key) {
        match encrypted_pad.decrypt_pad_to_blob(pool, master_key).await {
            Ok(blob) => Ok((
                ...
                FileFormat::from_bytes(&blob).media_type().to_string(),
            )),
...
#[tauri::command]
async fn get_node_tree(state: tauri::State<'_, Arc<Mutex<Cipherpad>>>) -> Result<NodeTree, String> {
    let mut cipherpad = state.inner().lock().await;
    if cipherpad.is_connected() {
        match cipherpad.get_node_tree().await {
...
#[tauri::command]
async fn get_pad_map(state: tauri::State<'_, Arc<Mutex<Cipherpad>>>) -> Result<PadMap, String> {
    let cipherpad = state.inner().lock().await;
    if cipherpad.is_connected() {
    ...
    if let Some(pool) = &cipherpad.pool {
        if let Some(master_key) = &cipherpad.master_key {
            if let Some(encrypted_pad) = cipherpad.pad_map.pads.get(&id) {
                match encrypted_pad
                    .clone()
                    .decrypt_pad_data(master_key, pool)
                    .await
                {
