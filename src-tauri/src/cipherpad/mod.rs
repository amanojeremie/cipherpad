...
use self::{
    crypto::{KEY_SIZE, SALT_SIZE},
    db::{value_from_sql, DatabasePool, SqlParamsBuilder},
    utils::{create_temp_file, CHUNK_SIZE},
};
...
impl EncryptedPad {
    ...
    pub async fn clear_pad_data(self, pool: &DatabasePool) -> Result<(), anyhow::Error> {
        pool.execute_query(
            "UPDATE node SET pad_data = ZEROBLOB(0) WHERE id = ?1",
            SqlParamsBuilder::new().add_param(self.id).build(),
        )
        .await?;
        Ok(())
    ...
    pub async fn encrypt_and_append_pad_data(
        self,
    ...
    pub async fn encrypt_file_to_pad(
        self,
        pool: &DatabasePool,
        master_key: &[u8],
        file: &str,
        ...
        loop {
            ...
            let encrypted_data_len = self
                .clone()
                .encrypt_and_append_pad_data(
        ...
        let encrypted_blob_pad_metadata =
            crypto::encrypt(blob_pad_metadata.as_bytes(), master_key)?;

        if encrypted_encrypted_chunk_sizes.len() + temp_file_metadata.len() as usize
            >= MAX_BLOB_SIZE
        {
      ...
      WHERE id = ?3",
            SqlParamsBuilder::new()
                ...
                .add_param(
                    encrypted_encrypted_chunk_sizes.len() + temp_file_metadata.len() as usize,
                )
                .add_param(self.id)
                .build(),
        )
    ...
    pub async fn decrypt_pad_to_file(
        self,
        pool: &DatabasePool,
        master_key: &[u8],
        file_to_create: &str,
        ...
        pool.open_blob(row_id, "pad_data", "node", true, move |blob| {
            ...
            let mut encrypted_encrypted_chunk_sizes =
                vec![0u8; blob_pad_metadata.encrypted_data_offset];
            reader.read(&mut encrypted_encrypted_chunk_sizes)?;
            let encrypted_chunk_sizes_bytes =
                crypto::decrypt(&encrypted_encrypted_chunk_sizes, &master_key)?;
            let mut encrpyted_chunk_sizes = Vec::<u64>::new();
            ...
            for encrypted_chunk_size in encrpyted_chunk_sizes {
                let mut encrypted_chunk = vec![0u8; encrypted_chunk_size.try_into().unwrap()];
    ...
    pub async fn decrypt_pad_to_blob(
        self,
        pool: &DatabasePool,
        master_key: &[u8],
        ...
        let blob_pad_metadata = self.clone().get_blob_pad_metadata()?;
        let blob = pool
            .open_blob(row_id, "pad_data", "node", true, move |blob| {
                let mut reader = std::io::BufReader::new(blob);
                let mut encrypted_encrypted_chunk_sizes =
                    vec![0u8; blob_pad_metadata.encrypted_data_offset];
                reader.read(&mut encrypted_encrypted_chunk_sizes)?;
...
fn populate_node_map(
    ...
    while let Some(current_node) = process_stack.pop() {
        ...
        for (&child_id, &_parent_id) in node_db_map
            .iter()
            .filter(|&(_cid, &pid)| pid == current_node_id)
        {
...
impl Cipherpad {
    ...
    pub async fn create_connection_pool(database_url: &str) -> Result<Self, anyhow::Error> {
        ...
        pool.execute_query("PRAGMA foreign_keys = ON;", vec![])
            .await?;
        pool.execute_query("PRAGMA auto_vacuum = FULL;", vec![])
            .await?;
        Ok(Self {
            pool: Some(pool),
            pad_map: PadMap::new(),
    ...
    pub async fn derive_master_key(&mut self, password: &str) -> Result<(), anyhow::Error> {
        if let Some(ref pool) = self.pool {
            let select_master_key_result = pool
                .select_query_single(
                    "SELECT master_key_salt FROM cipherpad WHERE id = 1;",
            ...
            let salt = match select_master_key_result {
                ...
                Err(_) => {
                    ...
                    pool.execute_query(
                        "INSERT INTO cipherpad (master_key_salt) VALUES (?1);",
                        SqlParamsBuilder::new().add_param(salt).build(),
                    )
                    .await?;
                    Ok(salt)
                }
    ...
    pub async fn get_node_tree(&mut self) -> Result<NodeTree, anyhow::Error> {
        if let Some(ref pool) = self.pool {
            if let Some(master_key) = &self.master_key {
                ...
                for node in nodes {
                    let id =
                        value_from_sql::<Uuid>(node.get(0)).context("Failed to read Uuid id")?;
                    let parent_id = value_from_sql::<Option<Uuid>>(node.get(1))
                        .context("Failed to read Uuid id")?;

                    let pad_metadata_encrypted = value_from_sql::<Vec<u8>>(node.get(2))
                        .context("Failed to read Vec metadata")?;

                    let encrypted_pad =
                        EncryptedPad::new(id, parent_id, pad_metadata_encrypted, master_key)?;
