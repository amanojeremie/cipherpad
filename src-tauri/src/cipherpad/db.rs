...
impl DatabasePool {
    ...
    pub async fn select_query_single(
        &self,
        query: &str,
        params: SqlParams,
        columns: usize,
    ...
    pub async fn begin_transaction(&self) -> Result<(), anyhow::Error> {
        self.execute_query("BEGIN TRANSACTION;", SqlParamsBuilder::new().build())
            .await?;
        Ok(())
    }

    pub async fn commit(&self) -> Result<(), anyhow::Error> {
        self.execute_query("COMMIT;", SqlParamsBuilder::new().build())
            .await?;
        Ok(())
    }

    pub async fn open_blob<F, R>(
        &self,
        row_id: i64,
        column: &str,
        table: &str,
